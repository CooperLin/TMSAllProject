...
#define APIRouteByCoordinate @"/itravel/itravelAPI/ExpoAPI/NearByStop.aspx?Lon=%0.6f&Lat=%0.6f&Range=%ld"
#define APIRouteByCoordinateEn @"/itravel/itravelAPI/ExpoAPI/NearByStop.aspx?Lon=%0.6f&Lat=%0.6f&Range=%ld&Lang=En"
...
//    cllocationHere = [[CLLocation alloc]initWithLatitude:24.1399  longitude:120.6776];
    NSString *str = isCht?APIRouteByCoordinate:APIRouteByCoordinateEn;
    dictionaryAPI = @{
                      @1:[NSString stringWithFormat:str,cllocationHere.coordinate.longitude,cllocationHere.coordinate.latitude,(long)integerSelectedDistance],
                      @"server":APIServer
                      };
    [self SendQueryRequest:1];
}
...
//    cllocationHere = [[CLLocation alloc]initWithLatitude:24.179  longitude:120.645];
    ...
    if(cllocationQuery == cllocationHere && cllocationQuery.coordinate.latitude > 0.0 && cllocationQuery.coordinate.longitude > 0.0)
    {
        [self actQueryData];
    }
}
